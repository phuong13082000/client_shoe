...
const getProducts = () =>
    axios.get(`${SHOE_API_BASE_URL}/shoes`);
...
const addToCart = (name: string, image: string, desc: string, price: string, quantity: number) =>
    axios.post(`${BASE_URL}/add-to-cart`, {name, image, desc, price, quantity}, headers);

const addToWishlist = (name: string, image: string, desc: string, price: string, rating: number) =>
    axios.post(`${BASE_URL}/add-to-wishlist`, {name, image, desc, price, rating}, headers);
